...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///manajemen_pengguna.db'
db = SQLAlchemy(app)
...
def create_tables():
    with app.app_context():
        db.create_all()
        if User.query.count() == 0:  # Periksa jika tabel kosong
            admin_user = User(
                username="admin",
                role="Admin",
...
if _name_ == '_main_':
    create_tables()  # Panggil fungsi untuk membuat tabel dan data awal
    app.run(debug=True)